...
from info import ADMINS # Imported for the instatus check

# Max messages to delete in one go
...
def instatus(client, message):
    user = client.get_chat_member(message.chat.id, message.from_user.id)
    # Note: Checking for 'ADMINS' as a string is likely a bug, 
    # but I am preserving the original logic.
